...
namespace SADST
{
    internal class FileHandler
    {
        ...
        {
            var path = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName);
        ...
        private static string GetSADSTPath()
        {
            return Path.Combine(Environment.CurrentDirectory, "SADST");
        }
        ...
        public bool OpenRpt()
        {
            ...
            file = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName, file);
        ...
        {
            var file = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName, ActiveProfile.ConsoleLogfile);
            if (File.Exists(file))
            {
                Process.Start("notepad.exe", file);
                return true;
        ...
        {
            var file = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName, ActiveProfile.RankingFile);
            if (File.Exists(file))
            {
                Process.Start("notepad.exe", file);
                return true;
        ...
        {
            var file = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName, ActiveProfile.PidFile);
            if (File.Exists(file))
            {
                Process.Start("notepad.exe", file);
                return true;
        ...
        {
            var path = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName);
            var file = Path.Combine(path, "SADST_config.cfg");
        ...
        {
            var path = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName);
            var file = Path.Combine(path, "SADST_basic.cfg");
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
        ...
        {
            var path = Path.Combine(GetSADSTPath(), ActiveProfile.ProfileName, "Users", ActiveProfile.ProfileName);
        ...
        {
            var path = Path.Combine(Environment.CurrentDirectory, "SADST", ActiveProfile.ProfileName);
            var configFile = Path.Combine(path, "SADST_config.cfg");
            var basicConfigFile = Path.Combine(path, "SADST_basic.cfg");
        ...
        {
            var path = Path.Combine(Environment.CurrentDirectory, "SADST", profileName);
        ...
        {
            var folder =
                new DirectoryInfo(Path.Combine(Environment.CurrentDirectory, "SADST", ActiveProfile.ProfileName));
